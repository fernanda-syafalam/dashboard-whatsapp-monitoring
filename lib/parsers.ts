...
export const parseCreatedAtToDateRange = (
  ...
  if (createdAtArray.length === 0) {
    if (options) {
      return {
        startDate: options.intialStartDate ? format(options.intialStartDate, 'yyyy-MM-dd') : '',
        endDate: options.initialEndDate ? format(options.initialEndDate, 'yyyy-MM-dd') : ''
      };
    }
  ...
  return {
    startDate: format(new Date(Number(startDate ?? thirtyDaysAgo)), 'yyyy-MM-dd'),
